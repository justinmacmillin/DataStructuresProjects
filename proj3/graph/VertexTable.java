...
public class VertexTable {
  ...
  public VertexTable resize() {
    ...
    VertexTable newTable = new VertexTable(this.size);
    if (loadFactor < 0.25 || loadFactor >= 1) {
      if (loadFactor < 0.25) {
        newTable = new VertexTable(halfLoad);
      }
      if (loadFactor >= 1) {
        newTable = new VertexTable(doubleLoad);
      }
      for (DList i : this.table) {
        if (i != null && i.getSize() > 0) {
          DListNode current = (DListNode) i.front();
          for (int n = 0; n < i.getSize(); n++) {
            ((VertexTable)newTable).insert((Vertex)current.item);
            current = current.next;
          }
        }
      }
    }
    return ((VertexTable)newTable);
  }
  ...
  public Vertex insert(Vertex v) {
   if (this.totalItems > 0 && this.size > 0) {
  	...
  	int index = compFunction(v.getVertex().hashCode());
